...
function _checkOrientation(iWidth,iHeight){
    if(false || s_bMobile && ENABLE_CHECK_ORIENTATION){
        if( iWidth>iHeight ){ 
...
function randomIntBetween(minValue,maxValue,precision){
    ...
    return Math.round(Math.min(minValue + (Math.random() * (maxValue - minValue)),maxValue).toFixed(precision));
}
